    ...
    def __init__(self, vn, cache: Cache = MemoryCache(),
        ...
        def setup_train():
            ...
            if project_id_api and dataset_id_api is not None:
                schema_query = f"""SELECT table_catalog, table_schema, table_name, column_name, data_type, description as `comment` FROM `{project_id_api}.{dataset_id_api}.INFORMATION_SCHEMA.COLUMN_FIELD_PATHS` WHERE table_name = '{table_api}'"""
